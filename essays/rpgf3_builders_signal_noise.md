...
> Originally published on [Mirror](https://mirror.xyz/cerv1.eth/CGS5QsqoX9k5_puYopug4SWODm06OAGwOPWiEil2v0U). Archived here from Arweave (tx `DbnTEzC6QyW-PpURBhamVDUVw8RvjQy6JfdIbAJuHXQ`).
...
![The OP token price rose from around 1.4 to 3.9 during the round](../assets/img/rpgf3_builders_signal_noise_1.png)
...
![The RetroPGF3 token distribution was relatively flat outside the top 50](../assets/img/rpgf3_builders_signal_noise_2.png)
...
![Most projects that applied ended up in the "peanut butter spread" part of the distribution](../assets/img/rpgf3_builders_signal_noise_3.png)

In Round 2 (below), we saw a more traditional power law distribution with a steady decay pattern all the way to the end.

![The RetroPGF2 distribution had a different curve shape](../assets/img/rpgf3_builders_signal_noise_4.png)
...
At [Open Source Observer](), we made a very strong push to include every project that had at least one open source repo associated with it in our dataset. Most projects included a public GitHub repo in their application, but some didn’t, so we had to track down their repo information manually (or they sent us a PR).
...
![Projects covered by Open Source Observer received over 80% of tokens](../assets/img/rpgf3_builders_signal_noise_5.png)
...
Now let’s apply this same type of distribution analysis to some more specific domains. These are the signals that badgeholders and engaged community members should pick up on.

One of the best places to look for signal is in the public artifacts that projects are leaving behind.
...
![How categories of projects with similar work artifacts performed](../assets/img/rpgf3_builders_signal_noise_6.png)
...
Note that OSS projects with onchain contributions on OP Mainnet and/or developer libraries are *not* included here. This is effectively a bundle of all of the other types of OSS contributions.

![OSS projects with no direct onchain component did very well](../assets/img/rpgf3_builders_signal_noise_21.png)
...
![Decentralized infrastructure and tooling is a public good](../assets/img/rpgf3_builders_signal_noise_8.png)
...
<https://x.com/carl_cervone/status/1745564803165262155?s=20>

The distribution of funding to these projects looks like chunky peanut butter.

![Developer libraries on NPM did extremely well in RetroPGF 3](../assets/img/rpgf3_builders_signal_noise_9.png)
...
On the other hand, the allocations for projects that are generating actual sequencer fees on OP Mainnet were surprisingly low. T

![Projects with onchain deployments did not do as well (relative to other OSS projects)](../assets/img/rpgf3_builders_signal_noise_10.png)
...
![Less than 20% of the total funding pool went to these projects](../assets/img/rpgf3_builders_signal_noise_11.png)
...
![Substack creators likely earned more from RetroPGF than from reader subscribtions](../assets/img/rpgf3_builders_signal_noise_12.png)

Again, remember that these categories are mutually exclusive, so a project with work happening in a public GitHub repo ***and*** a Substack would not appear in the distribution above.

It’s also encouraging seeing a cohort of Dune data wizards getting recognized for their contributions.

![RetroPGF helped Dune creators get a nice ROI on their premium subscriptions](../assets/img/rpgf3_builders_signal_noise_13.png)
...
![Is the next rug pull survivor turned white hat somewhere in the long tail?](../assets/img/rpgf3_builders_signal_noise_14.png)
...
![There's a large home for all the projects that didn't have an obvious home for the purposes of impact tracking during RetroPGF3 ](../assets/img/rpgf3_builders_signal_noise_15.png)
...
![For badgeholders, these 210 projects were probably the hardest to assess and the most likely to be in your DMs](../assets/img/rpgf3_builders_signal_noise_16.png)
...
![Big teams received smaller per capita distributions of tokens](../assets/img/rpgf3_builders_signal_noise_17.png)
...
We called these types of metrics “[Impact Vector](https://mirror.xyz/cerv1.eth/qL9YKLN9-dBzM89qKaZYgHK2ccpZy2XLPz2Z1PObwCg)s”.
...
![All 8 impact vectors with the average tokens awarded for each performance bucket](../assets/img/rpgf3_builders_signal_noise_18.png)
...
![Ranking my v1 impact vectors](../assets/img/rpgf3_builders_signal_noise_19.png)
